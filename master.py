...
def sum_contrast(cube, algo_dict, fwhm=6, star_phot=1, dp=None):
    ...
    dprint(cube.shape)
    dprint(parangles)
    # view_spectra(cube[0], logAmp=True, show=False)
    # view_spectra(cube[:,0], logAmp=True)
...
def set_field_params():
    ...
    # ap.companion_xy = [[2.5,0], [0,3], [-3.5,0], [0,-4], [4.5,0], [0,5], [-5.5,0],[0,-6]]
    # ap.n_wvl_init = 8
    # ap.n_wvl_final = 16
    ap.contrast = [10**-3.5]
...
def make_fields_master(monitor=False):
    ...
    if monitor:
        ...
        print(sp.numframes, 'numframes')
        sim = mm.RunMedis('test', 'fields')
        observation = sim()
        fields = observation['fields']
        # tess = np.sum(fields, axis=2)
        # view_spectra(tess[0], logAmp=True, show=False)
        # view_spectra(tess[:,0], logAmp=True, show=True)
        ...
        # else:
        #     view_spectra(fields[0, :, 0], logAmp=True, show=False)
        #     view_spectra(fields[0, :, 1], logAmp=True, show=True)
        #     # view_spectra(fields[:, -1, 2], logAmp=True, show=False)
...
def get_form_photons(fields, comps=True):
    ...
    stackcube = np.zeros((len(fields), ap.n_wvl_final, dp.array_size[1], dp.array_size[0]))
    # dprint(fields.shape)
    for step in range(len(fields)):
        ...
        cube = mkids.make_datacube_from_list(step_packets, (ap.n_wvl_final,dp.array_size[0],dp.array_size[1]))
        stackcube[step] = cube
...
def get_obj_photons(fields):
    ...
    objcube = np.zeros((2, len(fields), ap.n_wvl_final, dp.array_size[1], dp.array_size[0]))
    for step in range(len(fields)):
        dprint(step)
        obj = range(2)
        for o in obj:
...
def eval_performance(stackcubes, dps, metric_vals, comps=True):
    ...
    view_spectra(maps, logAmp=True, vmin=-1e-7, vmax=1e-7, show=True)
    # from medis.plot_tools import quick2D
    # new_maps = []
    # for map in maps:
    #     quick2D(map, logAmp=True)
    #     dprint(map.shape)
...
def reformat_planets(fields):
    ...
    # view_spectra(tess[0], logAmp=True, show=False)
    double_cube = np.zeros((sp.numframes, ap.n_wvl_final, 2, sp.grid_size, sp.grid_size))
    ...
    mkids.save_fields(double_cube, fields_file=iop.fields)
    return double_cube
