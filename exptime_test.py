...
from medis.plot_tools import quick2D, indep_images, view_spectra
...
from master import get_unoccult_psf, eval_method
...
def get_form_photons(fields, comps=True):
    ...
    dprint(len(fields))
    stackcube = np.zeros((len(fields), ap.n_wvl_final, mp.array_size[1], mp.array_size[0]))
...
def make_figure4(comps=False):
    if not comps and __name__ == '__main__':
        ...
        dprint((sp.grid_size//2, psf_template.shape))
        # quick2D(np.sum(psf_template,axis=0), logAmp=True)
        star_phot = phot.contrcurve.aperture_flux(np.sum(psf_template,axis=0),[mp.array_size[0]//2],[mp.array_size[0]//2],lod,1)[0]#/1e4#/sp.numframes * 500
    ...
    ap.companion = True
    sp.numframes = int(num_exp)
    iop.fields = iop.testdir + '/HR8799_phot_tag%i_tar_%i_comps_exp.h5' % (sp.numframes, np.log10(ap.star_flux))

    wsamples = np.linspace(ap.wvl_range[0], ap.wvl_range[1], ap.n_wvl_final)
    scale_list = wsamples / (ap.wvl_range[1] - ap.wvl_range[0])

    iop.form_photons = iop.form_photons[:-4] +'_exp'
    iop.device = iop.device[:-4] + '_exp'
    tp.pix_shift = [[0, 0]]
    if __name__ == '__main__':
        ...
        for exptime in exptimes:
            ...
            iop.device = iop.device.split('_exp')[0] + f'_exp={ap.exposure_time}_comps={comps}.pkl'
            ...
            if comps:
                ...
                # quick2D(SDI, logAmp=True, show=False)
                maps.append(SDI)
        ...
        view_spectra(maps, logAmp=True, vmin=-1e-6, vmax=1e-6)
