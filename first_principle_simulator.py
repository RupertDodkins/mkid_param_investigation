...
from medis.plot_tools import loop_frames, quick2D, view_spectra, compare_images, indep_images, grid, fmt
...
def make_figure2(normalize_spec=False):
    ...
    if __name__ == "__main__":  # required for multiprocessing - make sure globals are set before though
        # fields = gpd.run_medis(realtime=False)[:sp.numframes]
        sim = mm.RunMedis('test', 'fields')
observation = sim()
fields = observation['fields']
        ...
        if os.path.exists(iop.form_photons):
            ...
            dprint((fields.shape))
            if not os.path.isfile(iop.device):
                MKIDs.initialize()

            with open(iop.device, 'rb') as handle:
                dp = pickle.load(handle)

            photons = np.empty((0, 4))
            dprint(len(fields))
            stackcube = np.zeros((sp.numframes, ap.n_wvl_final, mp.array_size[1], mp.array_size[0]))
            for step in range(len(fields))[:10]:
                dprint(step, fields.shape)
                spectralcubes = np.abs(fields[step, -1, :, :]) ** 2

                if normalize_spec:
                    spectralcubes = spectralcubes * sum(throughput/ap.n_wvl_final) / throughput[:, np.newaxis, np.newaxis]
                ...
                cube = pipe.make_datacube_from_list(step_packets, (ap.n_wvl_final, dp.array_size[0], dp.array_size[1]))
        ...
        # cube = pipe.make_datacube(stem, (mp.array_size[0], mp.array_size[1], ap.n_wvl_final))
        # quick2D(cube[0], vmin=1, logAmp=True):144
        # plt.figure()
        x = np.arange(48,52)
        y = np.arange(48,52)
        ...
        # dith_duration = np.floor(sp.numframes / len(tp.pix_shift))
        #
        ...
        # #     # quick2D(cube[0], logAmp=True, vmin=1)
        # #     # quick2D(datacube[0], logAmp=True, vmin=1 )
        #
...
def get_packets_plots(datacubes, step, dp, mp, plot=False):
    ...
    # quick2D(datacube[0], logAmp=True)
    # plt.plot(np.sum(datacube, axis=(1,2)))
    # plt.show(block=True
    FoV_datacubes = np.zeros((2, ap.n_wvl_init, mp.array_size[0], mp.array_size[1]))
    for d in range(2):
    ...
    # for d in range(2):
    #     view_spectra(FoV_datacubes[d], logAmp=True)
    if plot:
        ...
        ax1.text(0.05, 0.85, 'Device FoV', transform=ax1.transAxes, fontweight='bold',
                 color='w', fontsize=16, bbox=props)
    # else:
    #     view_spectra(np.sum(FoV_datacubes, axis=0), logAmp=True)
    ...
    object_photons = []
    for d in range(2):
        num_events = int(ap.star_flux * ap.sample_time * np.sum(FoV_datacubes[d]))
        if sp.verbose:
            dprint(f'star flux: {ap.star_flux}, cube sum: {np.sum(FoV_datacubes[d])}, num events: {num_events}')
...
def param_compare():
    ...
    param_names = ['numframes', 'array_size', 'array_size_(rebin)', 'pix_yield', 'dark_bright', 'R_mean', 'R_sig', 'g_mean', 'g_sig']#'g_mean_sig']# 'star_flux', 'exp_time'
